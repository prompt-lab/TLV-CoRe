...
class TLVModel(HFCLIPModel):
    def __init__(self, args, config, num_frames, num_sensors, add_time_attn, tube_size):
        ...
        if self.num_sensors > 1:
            self.touch_router = nn.Linear(config.vision_config.hidden_size, self.num_sensors, bias=False)
            self.sensor_centers = nn.Parameter(torch.randn(num_sensors, config.vision_config.hidden_size))
    ...
    def touch_model_forward(
        self,
        pixel_values: Optional[torch.FloatTensor] = None,
        sensor_ids: Optional[torch.LongTensor] = None,
        output_attentions: Optional[bool] = None,
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
    ) -> Union[Tuple, BaseModelOutputWithPooling]:
        ...
        # route_weight = nn.functional.softmax(self.touch_router(hidden_states), dim=-1, dtype=torch.float32).to(hidden_states.dtype)
        # for i in range(B):
        #     sensor_id = sensor_ids[i]
        #     print(f'router_weight: {route_weight[i,:,sensor_id].size()}')
        #     print(f'hiddens_states: {hidden_states[i, :, :].size()}')
        #     hidden_states[i, :, :] = hidden_states[i, :, :] + torch.unsqueeze(route_weight[i,:,sensor_id], -1) * hidden_states[i, :, :]
        sensor_loss = None
        if self.num_sensors > 1:
            route_weight = nn.functional.softmax(self.touch_router(hidden_states), dim=-1, dtype=torch.float32).to(hidden_states.dtype)
            # 创建索引来提取每个 sample 的对应 sensor_id 路由权重
            sensor_ids = sensor_ids.to(hidden_states.device)  # 确保在同一个 device 上
            batch_indices = torch.arange(B, device=hidden_states.device)

            # route_selected: [B, T]
            route_selected = route_weight[batch_indices, :, sensor_ids]  # 利用 broadcasting
            # 扩展成 [B, T, 1] 以便乘到 hidden_states 上
            route_selected = route_selected.unsqueeze(-1)

            # 更新 hidden_states: [B, T, D]
            hidden_states = hidden_states + route_selected * hidden_states

            hidden_states_pooled = hidden_states.mean(dim=1)
            sim_matrix = F.cosine_similarity(
                hidden_states_pooled.unsqueeze(1),  # [B, 1, D]
                self.sensor_centers.unsqueeze(0),     # [1, num_sensors, D]
                dim=-1
            ) / 0.05
            sensor_loss = F.cross_entropy(sim_matrix, sensor_ids)
    ...
    def encode_touch_rm_proj(self, touch, normalize: bool = False):
        touch_outputs, _ = self.touch_model(
            pixel_values=touch,
            return_dict=True,
        )
    ...
    def encode_feeling_touch_rm_proj(self, touch, normalize: bool = False):
        ...
        gelsightA_before_outputs, _ = self.touch_model(
            pixel_values=gelsightA_before,
            return_dict=True,
        )
        gelsightA_after_outputs, _ = self.touch_model(
            pixel_values=gelsightA_after,
            return_dict=True,
        )
        gelsightB_before_outputs, _ = self.touch_model(
            pixel_values=gelsightB_before,
            return_dict=True,
        )
        gelsightB_after_outputs, _ = self.touch_model(
    ...
    def encode_touch(self, touch, sensor_ids, normalize: bool = False):
        if touch.dim() > 3:
            touch = torch.squeeze(touch)
        touch_outputs, sensor_loss = self.touch_model(
            pixel_values=touch,
        ...
        return touch_embeds / touch_embeds.norm(p=2, dim=-1, keepdim=True) if normalize else touch_embeds, sensor_loss
    ...
    def feeling_forward(
        ...
        touch_features, _ = self.encode_touch(touch, normalize=True) if touch is not None else None
    ...
    def forward(
            ...
            phra_input_ids=None, phra_attention_mask=None, sensor_ids=None, total_steps=None, current_step=None
    ):
        touch_features, sensor_loss = self.encode_touch(touch, sensor_ids, normalize=True) if touch is not None else None
        vision_features = self.encode_vision(vision, normalize=True) if vision is not None else None
        curriculum_representation = self.learn_curriculum(args, touch_features, vision_features, total_steps, current_step)\
                                                                    if total_steps is not None else None
