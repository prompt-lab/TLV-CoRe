...
from model.process_clip import add_time_attn_block, convert_model_to_lora, set_global_value, resize_pos, convert_model_to_shared_adapters
...
def create_vat_model(args):
    ...
    model= TLVModel(args, config, args.num_frames, args.num_sensors, args.add_time_attn, args.tube_size)
    ...
    if args.use_shared_adapter:
        convert_model_to_shared_adapters(args, model)
        if is_master(args):
            logging.info(f"Successfuly convert model to adapter style.")
