...
class Adapter(nn.Module):
    def __init__(self, input_dim, shared_dim, shared_module):
        super().__init__()
        self.down = nn.Linear(input_dim, shared_dim).to(device)
        self.shared = shared_module.to(device)  # nn.Linear(shared_dim, shared_dim)，共享
        self.up = nn.Linear(shared_dim, input_dim).to(device)

    def forward(self, x):
        return self.up(self.shared(self.down(x)))


def insert_adapters(encoder, shared_modules, name="", start_layer=0):
    for i in range(start_layer, len(encoder.layers)):
        layer = encoder.layers[i]
        hidden_size = layer.mlp.fc2.out_features
        shared = shared_modules[i - start_layer]
        layer.adapter = Adapter(hidden_size, shared.in_features, shared)

        original_forward = layer.forward

        def forward_with_adapter(self, *args, **kwargs):
            output = original_forward(*args, **kwargs)
            if isinstance(output, tuple):  # e.g., (hidden_states, ...)
        ...
        # 替换 forward 方法（闭包中绑定当前层）
        layer.forward = forward_with_adapter.__get__(layer, layer.__class__)


def convert_model_to_shared_adapters(args, model):
    shared_dim = args.adapter_shared_dim  # e.g., 256

    # 视觉层数更少
    num_layers = len(model.text_model.encoder.layers)

    # input_dim = model.text_model.encoder.layers[0].output.dense.out_features

    # 创建共享的 shared 模块（每层一个）
    shared_modules = nn.ModuleList([
        nn.Linear(shared_dim, shared_dim) for _ in range(num_layers)
    ])

    # 插入适配器到语言、视觉、触觉编码器
    insert_adapters(model.text_model.encoder, shared_modules, name="text", start_layer=0)
    insert_adapters(model.vision_model.encoder, shared_modules, name="vision", start_layer=num_layers)

    if args.clip_type == "tlv":
        insert_adapters(model.touch_model.encoder, shared_modules, name="touch", start_layer=num_layers)

    # 可保存共享 adapter 到模型字段，便于保存/加载
    # model.shared_adapters = shared_modules

    if is_master(args):
        logging.info(f"Inserted shared adapters into text/vision/touch encoders.")
