...
MODEL_DICT = {"ViT-L-14": "/data/develop/smallz/TLV-Core/CLIP-ViT-L-14-DataComp.XL-s13B-b90K"}
CHECKPOINT_DICT = {"ViT-L-14": "/data/develop/smallz/TLV-Core/CLIP-ViT-L-14-DataComp.XL-s13B-b90K/pytorch_model.bin"}
...
def main(args):
    ...
    # if not args.convert_to_lora and not args.use_shared_adapter:
    #     for param in model.touch_model.embeddings.parameters():
    #         param.requires_grad = False
    #     for param in model.touch_model.pre_layrnorm.parameters():
    #         param.requires_grad = False
    ...
    for name, param in model.named_parameters():
        if "adapter" not in name and "touch_router" not in name and "sensor_centers" not in name:
            param.requires_grad_(False)


    # print(f'text_______________________')

    # for name, module in model.text_model.named_modules():
    #     if name == '':
    #         continue
    #     trainable = any(p.requires_grad for p in module.parameters(recurse=False))
    #     print(f"{name:<40} {module.__class__.__name__:<30} {str(trainable):<10}")

    # print(f'vision_______________________')
    ...
    # print(f'touch_______________________')

    # for name, module in model.touch_model.named_modules():
    #     if name == '':
    #         continue
    #     trainable = any(p.requires_grad for p in module.parameters(recurse=False))
    #     print(f"{name:<40} {module.__class__.__name__:<30} {str(trainable):<10}")
    ...
    # no_decay = lambda n, p: p.ndim < 2 or "bn" in n or "ln" in n or "bias" in n or \
    #                     'logit_scale' in n or 'class_embedding' in n or 'patch_embedding' in n
    # decay = lambda n, p: not no_decay(n, p)

    # # 修改为 adapter 参数筛选
    # adapter = lambda n, p: "adapter" in n
    # non_adapter = lambda n, p: not adapter(n, p)

    # named_parameters = list(model.named_parameters())

    # # 非 adapter 参数分组
    # no_decay_non_adapter_params = [[n, p] for n, p in named_parameters if no_decay(n, p) and non_adapter(n, p) and p.requires_grad]
    # decay_non_adapter_params = [[n, p] for n, p in named_parameters if decay(n, p) and non_adapter(n, p) and p.requires_grad]

    # # adapter 参数分组
    # no_decay_adapter_params = [[n, p] for n, p in named_parameters if no_decay(n, p) and adapter(n, p) and p.requires_grad]
    # decay_adapter_params = [[n, p] for n, p in named_parameters if decay(n, p) and adapter(n, p) and p.requires_grad]

    # param_groups = []
    # if no_decay_non_adapter_params:
    #     param_groups.append({"params": [p for n, p in no_decay_non_adapter_params], "weight_decay": 0., 'lr': args.lr * args.coef_lr})
    # if decay_non_adapter_params:
    #     param_groups.append({"params": [p for n, p in decay_non_adapter_params], "weight_decay": args.wd, 'lr': args.lr * args.coef_lr})
    # if no_decay_adapter_params:
    #     param_groups.append({"params": [p for n, p in no_decay_adapter_params], "weight_decay": 0.})
    # if decay_adapter_params:
    #     param_groups.append({"params": [p for n, p in decay_adapter_params], "weight_decay": args.wd})
    ...
    no_decay_params = [p for n, p in named_parameters if no_decay(n, p) and p.requires_grad]
    decay_params = [p for n, p in named_parameters if decay(n, p) and p.requires_grad]
